...
**ISO образ**. Для создания виртуальной машины необходим образ. В инструкции будет использоваться дистрибутив Ubuntu, который доступен на сайте https://ubuntu.com/.

## Загрузка образов в Proxmox

Чтобы установить ОС в виртуальную машину, нужен установочный ISO-файл этой ОС (например, образ Ubuntu Server или Windows). Необходимо скачать нужный ISO, а затем **загрузить его в хранилище Proxmox**. В веб-интерфейсе перейдите в **Storage (например, local) - ISO images** и нажмите кнопку **Upload**. Выберите файл ISO со своего ПК – начнется загрузка на сервер. По окончании ISO появится в списке Content хранилища. Убедитесь, что ISO образ доступен.
...
4. Настройка сетевого соединения
   1. Либо не меняем - IP будет получен автоматически
   2. Либо указываем  статический IP **прямо в установщике** - это проще, чем через Netplan после установки.  
Выбираем сетевой интерфейс - **Edit IPv4** - **Manual** (значения приведены для примера, указывайте свои реальные значения):
       1. **Subnet** `192.168.1.0/24` - Адрес подсети и маска: `/24` = 255.255.255.0, диапазон хостов 192.168.1.1-254
       2. **Address** `192.168.1.50` - Статический IP самого сервера внутри этой подсети
       3. **Gateway** `192.168.1.1` - Адрес роутера - шлюз по умолчанию для трафика за пределы подсети
       4. **Name servers** `192.168.1.1, 8.8.8.8` - DNS-резолверы: первый - локальный (роутер), резервный - публичный Google DNS  

1. Прокси - пропускаем.
2. Настройка адресов зеркал - не меняем.
3. Настройки жесткого диска - убираем галку `Setup this disk as an LVM group`.
4. Разметка диска - не меняем. Подтверждаем настройки.
5.  На экране установок профиля указываем пользователя и пароль (administrator, например). Имя компьютера рекомендую указать то же, что и имя указанное в Proxmox.
6.  Ubuntu Pro - не ставим, конечно.
7.  Установка SSH - ставим галку Install OpenSSH server.
8.  На экране со списком рекомендуемых для установки пакетов не выбираем ничего.
9.  Начнется процесс установки. После завершения установки система предложит выполнить логин.
